...
public class moveNotator {
	...
	moveNotator(Piece piece, boolean wasCapture, boolean wasCheck, boolean wasMate, boolean blackTop, Board board, Vector where, boolean wasQC, boolean wasKC){
		...
		spec:{
			...
			if(piece instanceof Knight) this.specKnightFile(where, (Knight)piece, board);
			if(specBoth) {
				moveNotation += this.getFile(piece.getPosition().getX(), blackTop);
	...
	public void specKnightFile(Vector pos, Knight knight, Board board) {
		int x = pos.getX(), y = pos.getY();
		for(int i=0; i<8; i++) {
			...
			if(board.isOccupied(x+Board.xC[i],y+Board.yC[i])) {
				...
				if(!board.validateMove(board.getSquare(x+Board.xC[i],y+Board.yC[i]), pos, false)) specFile = true;	
			}
		}		
	}
	...
	public String getRank(int y, boolean isBT) {
		if(!isBT) return Integer.toString(y+1);
		else return Integer.toString(8-y);
	}
